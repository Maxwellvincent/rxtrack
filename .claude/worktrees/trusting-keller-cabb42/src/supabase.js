...
// ─── ANKI BANK SYNC ────────────────────────────────────────
//
// The Anki Proper / Proper+ extracted bank lives bundled in src/ankiBank.json,
// but we also mirror it to Supabase so:
//   1. The data exists for terms/blocks the user hasn't yet created in-app.
//   2. The bank can be updated from one device and read on another without
//      a redeploy.
//   3. Cloud rows can later add lectures the bundled JSON doesn't have.
//
// Storage: one user_kv row per Anki lecture, key = "rxt-anki-lec-<slug>".
// Average row size ~50KB; max ~200KB for big lectures. Stays well within
// Supabase JSON column limits.
const ANKI_KV_PREFIX = "rxt-anki-lec-";
const ANKI_VERSION_KEY = "rxt-anki-bank-version";

function ankiSlug(lectureName) {
...
/**
 * Push the bundled Anki bank to Supabase, one row per lecture. Idempotent —
 * upserts on (user_id, key). Stores a version flag so subsequent calls skip
 * unless ankiBankVersion has changed.
 */
export async function pushAnkiBankToCloud(userId, ankiBankData, version = "v1") {
  if (!userId || !ankiBankData?.lectures) return { pushed: 0, skipped: true };
  const now = new Date().toISOString();
  try {
    // Check if this version already pushed
    const { data: ver } = await supabase
      .from("user_kv")
      .select("data")
      .eq("user_id", userId)
      .eq("key", ANKI_VERSION_KEY)
      .maybeSingle();
    if (ver?.data?.version === version) return { pushed: 0, skipped: true };

    const rows = Object.entries(ankiBankData.lectures).map(([lectureName, notes]) => ({
      user_id: userId,
      key: ANKI_KV_PREFIX + ankiSlug(lectureName),
      data: { lectureName, notes, version },
      updated_at: now,
    }));

    // Batch in chunks of 25 to keep request size sane
    let pushed = 0;
    for (let i = 0; i < rows.length; i += 25) {
      ...
      if (error) {
        console.warn("anki push slice failed:", error);
        break;
      }
      pushed += slice.length;
    }
    ...
    await supabase
      .from("user_kv")
      .upsert(
        [{ user_id: userId, key: ANKI_VERSION_KEY, data: { version, pushedAt: now }, updated_at: now }],
        { onConflict: "user_id,key" }
      );

    return { pushed, skipped: false };
  } catch (e) {
    console.warn("pushAnkiBankToCloud error:", e);
    return { pushed: 0, skipped: false, error: String(e) };
  }
}

/**
 * Pull all Anki lecture rows from Supabase. Returns a {lectureName: notes[]} map.
 * Used to merge cloud-only lectures into the in-memory bank — particularly useful
 * for terms/blocks not yet created in-app.
 */
export async function pullAnkiBankFromCloud(userId) {
  if (!userId) return {};
  try {
    const { data, error } = await supabase
      ...
      .select("key, data")
      .eq("user_id", userId)
      .like("key", ANKI_KV_PREFIX + "%");
    if (error) {
      console.warn("anki pull failed:", error);
      return {};
    }
    const out = {};
    (data || []).forEach((row) => {
      const lecName = row?.data?.lectureName;
      const notes = row?.data?.notes;
      if (lecName && Array.isArray(notes)) out[lecName] = notes;
    });
    return out;
  } catch (e) {
    console.warn("pullAnkiBankFromCloud error:", e);
    return {};
  }
}
