...
/**
 * Heuristic fallback for cognitiveType when the AI extractor doesn't tag.
 * Returns "physiology-calc" if the stem contains numeric inputs and asks for a
 * computed/measured value — covers the common school-exam calc patterns.
 */
function detectCalcCognitiveType(stem) {
  const s = String(stem || "").toLowerCase();
  if (!s) return null;
  const askPattern =
    /(which.*following.*values?|best (represents|approximates)|most likely.*value|calculate|compute|determine.*value|values? best|what is the.*(value|gradient|clearance|flow|volume|pressure|capacity))/i;
  const calcContext =
    /(minute ventilation|alveolar ventilation|dead space|a-a gradient|alveolar-arterial|pio2|pao2|fio2|pb\b|barometric|tidal volume|respiratory quotient|gfr|glomerular filtration|filtered load|clearance|renal blood flow|renal plasma flow|hematocrit|filtration fraction|net filtration pressure|body water|extracellular fluid|intracellular fluid|tbw|ecf|icf|compliance|transpulmonary|delta v|delta p|p_?ip|p_?a\b)/i;
  const hasNumbers = /\d/.test(s);
  if (hasNumbers && askPattern.test(s) && calcContext.test(s)) {
    return "physiology-calc";
  }
  return null;
}
...
async function parseWithAI(fullText, format, onProgress, examTitle = "") {
  ...
  for (let ci = 0; ci < chunks.length; ci++) {
    ...
    const prompt =
      ...
      '"cognitiveType":"physiology-calc|anatomy-landmark|mechanism-direction|histology-image-id|genetics-principle|pathophys-mechanism|cell-biology|test-method-selection|pharmacology|embryology"' +
      "}]}\n\n" +
      ...
      'Set cognitiveType to "physiology-calc" for ANY question that requires arithmetic to answer ' +
      "(minute ventilation, alveolar ventilation, A-a gradient, PIO2, dead space, GFR, clearance, " +
      "renal blood flow, filtered load, body water, ECF/ICF, compliance, transpulmonary pressure, " +
      "or any question whose stem provides numbers and asks for a numeric result). Otherwise pick " +
      "the best-fit value from the list.\n\n" +
      "TEXT:\n" +
      chunks[ci];
      ...
      for (const q of qs) {
        ...
        if (!seenStems.has(key) && q.stem && q.stem.length > 20) {
          ...
          allQuestions.push({
            ...
            cognitiveType: q.cognitiveType || detectCalcCognitiveType(q.stem) || null,
          });
        }
      }
    } catch (e) {
...
async function parseGridFormat(pages, onProgress, options = {}) {
  ...
  for (let gi = 0; gi < gridPageGroups.length; gi++) {
    ...
    const prompt =
      ...
      '- Set cognitiveType to "physiology-calc" for any question requiring arithmetic to answer ' +
      "(minute/alveolar ventilation, A-a gradient, PIO2, dead space, GFR, clearance, renal blood flow, " +
      "filtered load, body water, ECF/ICF, compliance, transpulmonary pressure, etc.). Otherwise pick the best-fit value.\n\n" +
